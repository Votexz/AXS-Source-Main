...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	var bgFadeGraphic:FlxSprite = null;
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		bgFadeGraphic = new FlxSprite(-500, -500).makeGraphic(Std.int(FlxG.width * 4.3), Std.int(FlxG.height * 4.3), 0xFFFD723C);
		bgFadeGraphic.scrollFactor.set();
		bgFadeGraphic.visible = true;
		//bgFadeGraphic.alpha = 0;

		bgFade = new FlxSprite(-430, -230).loadGraphic(Paths.image('dialogue_BG/bg1'));
		bgFade.setGraphicSize(Std.int(bgFade.width * 0.67));
		bgFade.updateHitbox();
		//bgFade.alpha = 0;
		add(bgFade);

		if (PlayState.SONG.song.toLowerCase() == 'broadcasting')
		{
			add(bgFadeGraphic);
		}
		if (PlayState.SONG.song.toLowerCase() == 'my-sweets')
		{
			bgFadeGraphic.color = 0xFF09002B;
			add(bgFadeGraphic);
		}
	...
	override function update(elapsed:Float)
	{
		...
		if(!dialogueEnded) {
			...
			if(bgFade.alpha > 0.5) bgFade.alpha = 0.9;

			bgFadeGraphic.alpha += 0.5 * elapsed;
			if(bgFadeGraphic.alpha > 0.5) bgFadeGraphic.alpha = 0.5;
			...
			if(bgFadeGraphic != null) {
				bgFadeGraphic.alpha -= 0.5 *elapsed;
				if(bgFadeGraphic.alpha <= 0) {
					bgFadeGraphic.kill();
					bgFadeGraphic.destroy();
					bgFadeGraphic = null;
				}
			}
			...
			if(box == null && bgFade == null && bgFadeGraphic == null) {
				for (i in 0...arrayCharacters.length) {
